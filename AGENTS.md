...
- Release/image tags must match `erpverein-v<erpnext-version>-<app-version>`, for example `erpverein-v16.28.0-0.1.8`. Tag pushes derive the ERPNext tag and app ref from that release tag while keeping Frappe and `frappe_docker` pinned; manual image tags are sanitized.
