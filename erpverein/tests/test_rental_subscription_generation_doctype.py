...
class TestRentalSubscriptionGenerationDoctype(IntegrationTestCase):
    def test_run_doctype_has_background_processing_status(self):
        ...
        self.assertIn("Vorschau mit Fehlern", meta.get_field("status").options.split("\n"))
        self.assertEqual(meta.get_field("worker_job_id").fieldtype, "Data")
        self.assertEqual(meta.get_field("finished_at").fieldtype, "Datetime")
    ...
    def test_preview_child_table_fields_exist(self):
        ...
        self.assertEqual(meta.get_field("generation_key").fieldtype, "Data")
