...
def normalize_mieter(doc) -> None:
    ...
    doc.kontoinhaber = normalize_text(doc.get("kontoinhaber"))
    doc.iban = normalize_iban(doc.get("iban"))
    doc.bic = normalize_text(doc.get("bic"))
    doc.bank_name = normalize_text(doc.get("bank_name"))
...
def validate_mieter_billing(doc) -> None:
    ...
    missing = [
        ...
        for fieldname, label in {
            ...
            "iban": _("IBAN"),
            "bank_name": _("Name der Bank"),
        }.items()
        if not doc.get(fieldname)
    ]
    if missing:
        frappe.throw(_("Fuer Lastschrift fehlen: {0}").format(", ".join(missing)))
...
def validate_mieter_customer_link(doc) -> None:
    ...
    if other_mieter:
        frappe.throw(
            _("Kunde {0} ist bereits mit Mieter {1} verknuepft.").format(
                frappe.bold(current_customer), frappe.bold(other_mieter)
            )
        )
    ...
    if linked_mieter and linked_mieter != doc.name:
        frappe.throw(
            _("Kunde {0} verweist bereits auf Mieter {1}.").format(
                frappe.bold(current_customer), frappe.bold(linked_mieter)
            )
        )
...
def validate_customer_rental_link(doc, method: str | None = None) -> None:
    ...
    if previous_mieter and previous_mieter != mieter:
    ...
    if linked_customer and linked_customer != doc.name:
        frappe.throw(
            _("Mieter {0} ist bereits mit Kunde {1} verknuepft.").format(
                frappe.bold(mieter), frappe.bold(linked_customer)
            )
        )
    ...
    if other_customer:
        frappe.throw(
            _("Mieter {0} ist bereits bei Kunde {1} eingetragen.").format(
                frappe.bold(mieter), frappe.bold(other_customer)
            )
        )
...
def sync_mieter_to_customer(doc) -> None:
    ...
    if current_customer:
        ...
        from verein_erp.services.mieter_customer_sync_service import sync_customer_from_mieter
